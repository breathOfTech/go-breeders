...
func (app *application) render(w http.ResponseWriter, t string, td *templateData) {
	...
	// if we are using the template cache, try to get the template from out map, stored in the receiver
	if app.config.useCache {
		if templateFomMap, ok := app.templateMap[t]; ok {
			tmpl = templateFomMap
		}
	}
	if tmpl == nil {
		...
		if err != nil {
			log.Println("error building template: %w", err)
			return
		}
